  ...
  for (i in 1:n.sites){  
    ...
    for (i in 2:(n.sites-1)){
      ...
      for (t in 2:n.months){ #12
        ...
        State[i,t] ~ dcat(ps[State[i,t-1], i, t-1, 1:n.states]) 
    ...
    D[n.sites,1] <- D.init[n.sites]
     ...
     for (t in 2:n.months){
        State[1,t] ~ dcat(ps[State[1,t-1], 1, t-1, 1:n.states])
        State[n.sites,t] ~ dcat(ps[State[n.sites,t-1], n.sites, t-1, 1:n.states])
        D[1,t] <- State[2,t]
    ...
    for(h in 1:n.obs){
      ...
      y2[site.obs2[h]] ~ dcat(po_dat2[State.fin[site.obs2[h]], site.obs2[h], n.months-1, 1:2])
      
      y3[site.obs3[h]] ~ dcat(po_dat3[State.fin[site.obs3[h]], site.obs3[h], n.months-1,1:3])
...
rmods <- rep(NA,n.sims)
confs <- rep(NA,n.sims)
rmcmcs <- rep(NA,n.sims)
cmods <- rep(NA,n.sims)
cmcmcs <- rep(NA,n.sims)
...
# State transition
  ...
  my.constants <- list(n.sites = n.sites,
       n.months = n.months, 
       n.obs = n.obs, 
       n.states = n.states,
  ...
  for(s in 1:n.sims){
    my.data[[s]] <- list(S.init = S.init[,year,s],
  ...
  initial.values <- list(State = State.init)
  
  for(s in 1:n.sims){
    rmods[s] <- paste("Rmodel", s, sep = "_")
    
    #1. Compile model:
    assign(rmods[s], 
           nimbleModel(code = modcode, constants = my.constants, 
                       data = my.data[[s]],check = FALSE, calculate = FALSE,
                       inits = initial.values))
    
    confs[s]<- paste("conf", s, sep = "_")
    #2. configure model
    assign(confs[s],
           configureMCMC(get(rmods[s]), monitors = parameters.to.save,
                         control = list(maxContractions = 1000)))
    
    rmcmcs[s]<- paste("Rmcmc", s, sep = "_")
    
    #3. Build MCMC
    assign(rmcmcs[s],
           buildMCMC(get(confs[s])))
    
    cmods[s]<- paste("Cmodel", s, sep = "_")
    
    #4. Compile model
    assign(cmods[s], 
           compileNimble(get(rmods[s]), showCompilerOutput = FALSE))
    
    cmcmcs[s]<- paste("Cmcmc", s, sep = "_")
    
    #5. Compile mcmcs
    assign(cmcmcs[s], 
           compileNimble(get(rmcmcs[s]), project=get(cmods[s])))
  ...
  for(s in 1:S){
    assign(outs[s],
           runMCMC(get(cmcmcs[s]), niter = n.iter , nburnin = n.burnin , nchains = n.chains,
                   inits = initial.values, summary = TRUE,
                   setSeed = FALSE, progressBar = TRUE, samplesAsCodaMCMC = FALSE))
    
  }
  
  # #------------------------Year 1+ Running Nimble ------------------------#
  # 
  # for(s in 1:S){
  #   #set data for this years run:
  #   get(rmods[s])$setData(my.data[[s]])
  #   
  #   #set initial values
  #   get(rmods[s])$setInits(initial.values[[s]])
  #   
  #   #1. Configure model
  #   confs[s]<- paste("conf", s, sep = "_")
  #   assign(confs[s],
  ...
  #   assign(outs[s],
  #          runMCMC(get(cmcmcs[s]), niter = n.iter , nburnin = n.burnin , nchains = n.chains,
  #                  inits = initial.values[[s]], summary = TRUE,
  #                  setSeed = FALSE, progressBar = TRUE, samplesAsCodaMCMC = FALSE))
  # }
  ...
  test2 <- mcmc.list(as.mcmc(out_2$samples$chain1),as.mcmc(out_2$samples$chain2), as.mcmc(out_2$samples$chain3))
  vals <- gelman.diag(test2, multivariate = F)$psrf[,1]
