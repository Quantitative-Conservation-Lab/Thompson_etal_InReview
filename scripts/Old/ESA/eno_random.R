...
#### Path Name ####
path <- here::here("results", "ESA", "eno_random")
res <- c('results/ESA/eno_random') 
...
for(s in 1:n.sims){
  ...
  sites.rem[,year,s] <- sample(seq(1,n.sites), n.sites, replace = F)
}}
...
file_name = paste(path, 'States_eno_random.csv',sep = '/')
...
file_name = paste(path, 'Mean.States_eno_random.csv',sep = '/')
...
file_name = paste(path, 'y.obs_eno_random.csv',sep = '/')
write.csv(y.obs.df,file_name)


rem.site.df <- y.obs.df %>% filter(observed.state == 1)
file_name = paste(path, 'rem.site_eno_random.csv',sep = '/')
...
file_name = paste(path, 'sites.visit_eno_random.csv',sep = '/')
