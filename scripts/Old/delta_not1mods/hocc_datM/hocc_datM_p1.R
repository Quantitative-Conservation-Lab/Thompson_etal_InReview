...
res <- c('results/Multistate/hocc_datM_p1') 
      ...
      for(i in sites.rem.M[,week,year,s]){ #order of sites where removal occurs
        ...
        if(resource.total[week,year,s] < n.resource){
          ...
          if(yM[i,1,week, year, s] > 1){ 
            ...
            resource.total[week,year,s] <- resource.total[week,year,s] + search.hours + removal.hours[yM[i,1,week, year, s]]
            ...
            if(yM[i,2, week, year, s] > 1){ #if seen (state observed > 1) the second time
              ...
              resource.total[week,year,s] <- resource.total[week,year,s] + 2*search.hours + removal.hours[yM[i,2,week, year, s]]
            } 
