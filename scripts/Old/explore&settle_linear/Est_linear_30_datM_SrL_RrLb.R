...
for(year in 2:n.years){
  ...
  if(year == last.explore){
    for(s in 1:n.sims){
      ...
      logsearch.effort.L[s] <- (logit(0.2) - B0.p.l.est[s])/(B1.p.l.est[s])
      logsearch.effort.H[s] <- (logit(0.2) - B0.p.h.est[s])/(B1.p.h.est[s])
      ...
      removal.L[s] <- (logit(0.2) - B0.eps.l.est[s])/(B1.eps.l.est[s])
      removal.H[s] <- (logit(0.2) - B0.eps.h.est[s])/(B1.eps.h.est[s])
