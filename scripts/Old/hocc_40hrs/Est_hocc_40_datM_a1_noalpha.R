...
#------------------------------------------------------------------------------#
#### Path to save data ####
path <- here::here("results", "explore_settle", "noexplore_a1")
res <- c('results/explore_settle/noexplore_a1"') #subset of path for plot save
#------------------------------------------------------------------------------#
#### Management Strategy ####
...
B0.gamma <- B1.gamma <- B2.gamma  <-  B0.eps.l <- B0.eps.h <- B0.phi.h <-
 ...
 delta <- epsB.l <-epsB.h <-  phiB.l <- phiB.h <- g <- rep(NA, n.params)
...
pM.l <- invlogit(B0.p.l + B1.p.l*logsearch.effort) #low state detection 
pM.h <- invlogit(B0.p.h + B1.p.h*logsearch.effort) #high state detection 
