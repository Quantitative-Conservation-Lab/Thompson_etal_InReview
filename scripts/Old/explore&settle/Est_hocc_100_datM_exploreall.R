...
#------------------------------------------------------------------------------#
#### Path to save data ####
path <- here::here("results", "explore_settle", "exploreall_more")
res <- c('results/explore_settle/exploreall_more') #subset of path for plot save
#------------------------------------------------------------------------------#
#### Management Strategy ####
load("parameters.RData")
#rule = by highest estimated state

n.resource <- 100 #total hours per week
...
P.datM <- array(NA, dim = c(n.states, n.params, n.sites, n.weeks, n.years, n.sims, n.states))
...
for(w in 1:n.weeks){
  for(y in 1:n.years){
    for(s in 1:n.sims){
      pM.l[1:n.sites,w,y,s] <- invlogit(B0.p.l[p] + B1.p.l[p]*log(hours.dat[1,1:n.sites,w,y,s])) #low state detection 
      pM.h[1:n.sites,w,y,s]  <- invlogit(B0.p.h[p] + B1.p.h[p]*log(hours.dat[1,1:n.sites,w,y,s])) #high state detection 
      
      for(i in 1:n.sites){
        P.datM[1,p,i,w,y,s,] <- c(1,0,0)
        P.datM[2,p,i,w,y,s,] <- c(1-pM.l[i,w,y,s], pM.l[i,w,y,s], 0)
        P.datM[3,p,i,w,y,s,] <- c(1-pM.h[i,w,y,s], pM.h[i,w,y,s]*(1-delta[p]), pM.h[i,w,y,s]*delta[p])
      }
    }
  }
}
...
for(year in 2:n.years){
  ...
  for(s in 1:n.sims){
    for(week in 1:4){
      ...
      if(week > 1){
        ...
        eps.l[,week,y,p,s] <- invlogit(B0.eps.l[p] + B1.eps.l[p]*prev.rem.vec*hours.dat[2,i,week,y,s]) #low eradication
        eps.h[,week,y,p,s] <- invlogit(B0.eps.h[p] + B1.eps.h[p]*prev.rem.vec*hours.dat[2,i,week,y,s]) #high eradication
        
        #transition rates
        phi.h[,week,y,p,s] <- invlogit(B0.phi.h[p] - B1.phi.h[p]*prev.rem.vec*hours.dat[2,i,week,y,s])
      ...
      for(i in sites.rem.M[,week,y,p,s]){ #order of sites where removal occurs
        which.site <- which(sites.rem.M[,week,y,p,s] == i)
        
        #A. while we still have resources to spend:
        if(resource.total[week,y,p,s] < (n.resource- max.spent[i,week,y,s])){
          
          #1. first occasion occupancy data (1 = not detected, 2 = detected)
          yM[i,1,week, y,p, s] <- rcat(1, P.datM[State[i,week,y,p,s],p,i,week,y,s,])
          ...
          if(yM[i,1,week, y, p,s] > 1){ 
            ...
            if(i == sites.rem.M[1,week,y,p,s]){
              #Calculating resources used = resources already used + search hours + removal hours
              resource.total[week,y,p,s] <- resource.total[week,y,p,s] + hours.dat[1,i,week,y,s] + hours.dat[2,i,week,y,s]
            }else{
              resource.total[week,y,p,s] <- resource.total[week,y,p,s] + hours.dat[1,i,week,y,s] + hours.dat[2,i,week,y,s] + 
                0.1*abs(sites.rem.M[which.site,week,y,p,s]  -sites.rem.M[which.site-1,week,y,p,s]) #hours traveling 
            }

          }else{
            #2b. If not seen the first occasion, we need to search again:
            #Second occasion occupancy data
            yM[i,2, week, y, p,s] <- rcat(1, P.datM[State[i,week,y,p,s],p,i,week,y,s,])
            ...
            if(yM[i,2, week, y,p, s]==1){ #if not seen (state observed = 1)
              ...
              if(i == sites.rem.M[1,week,y,p,s]){
                ...
                resource.total[week,y,p,s] <- resource.total[week,y,p,s] + hours.dat[1,i,week,y,s]
              }else{
                resource.total[week,y,p,s] <- resource.total[week,y,p,s] + hours.dat[1,i,week,y,s] + 
                  0.1*abs(sites.rem.M[which.site,week,y,p,s]  -sites.rem.M[which.site-1,week,y,p,s]) #hours traveling 
              } 
  ...
  truth.params <- c(B0.eps.l[p], B1.eps.l[p], B0.eps.h[p], B1.eps.h[p], B0.phi.h[p], B1.phi.h[p],
                    ...
                    B0.p.l[p], B1.p.l[p], B0.p.h[p], B1.p.h[p], delta[p])
...
#### uncomment ####
  ...
  ###### 3c. Update efforts #####
  if(year == last.explore & n.years != last.explore){
    for(s in 1:n.sims){
    ...
    for(s in 1:n.sims){
     ...
     # p <- which(apply(params, 1, function(x) return(all(x == c(3,3,3,3,3,3))))) #bad invasion but good management
      ...
      pM.l[1,1,(last.explore+1),s] <- invlogit(B0.p.l[p] + B1.p.l[p]*logsearch.effort[s]) #low state detection 
      pM.h[1,1,(last.explore+1),s]  <- invlogit(B0.p.h[p] + B1.p.h[p]*logsearch.effort[s]) #high state detection 
      
      for(i in 1:n.sites){
        for(w in 1:n.weeks){
          for(y2 in (last.explore+1): n.years){
            P.datM[1,p,i,w,y2,s,] <- c(1,0,0)
            P.datM[2,p,i,w,y2,s,] <- c(1-pM.l[1,1,(last.explore+1),s], pM.l[1,1,(last.explore+1),s], 0)
            P.datM[3,p,i,w,y2,s,] <- c(1-pM.h[1,1,(last.explore+1),s],pM.h[1,1,(last.explore+1),s]*(1-delta[p]), pM.h[1,1,(last.explore+1),s]*delta[p])
          }
        } 
      }
...
truth.params <- c(B0.eps.h[p], B0.eps.l[p], B0.gamma[p], B0.p.h[p],B0.p.l[p],
