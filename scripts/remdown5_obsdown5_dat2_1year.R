...
for (i in 1:n.sites){  
  ...
  for (t in 1:n.months){
    ...
    #invaded to empty
    ps[2,i,t,1] <- (eps*rem.vec[i]) #erradication probability
    
    #invaded to invaded
    ps[2,i,t,2] <- 1- (eps*rem.vec[i]) #erradication failure probability
    ...
    po[2,i,t,1] <- 1-p #not detected probability
    
    #invaded observed present
    po[2,i,t,2] <- p #detection probability
...
#### Path Name ####
path <- here::here("Final_Project", "results", "remdown5_obsdown5")
res <- c('Final_Project/results/remdown5_obsdown5') #subset of path for plot save
...
for(year in 1:years){
  ...
  for(s in 1:n.sims){
    if(length(site.rem.options[[s]]) < n.rem){
      ...
      site.rem[,year+1,s] <- tail(which(final.states.mean[,year,s] == 2),n.rem)
    }
...
file_name = paste(path, 'rdown5_odown5_gamma0est.csv',sep = '/')
...
file_name = paste(path, 'rdown5_odown5_gamma1est.csv',sep = '/')
...
file_name = paste(path, 'rdown5_odown5_epsest.csv',sep = '/')
...
file_name = paste(path, 'rdown5_odown5_pest.csv',sep = '/')
...
file_name = paste(path, 'rdown5_odown5_segrem.csv',sep = '/')
