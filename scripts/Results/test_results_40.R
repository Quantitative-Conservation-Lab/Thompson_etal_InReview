...
path <- here::here("results", "test_40")
...
ggplot(param.bias1) + 
  ...
  xlab("Year")+ ylab("Relative Bias for Estimating Parameter")+
  facet_wrap(~param2)
...
ggplot(S_generating_final2) +
  ...
  scale_color_manual(name = "Hours spent searching and removing at a site", labels = alt.hours, values = colors) +
  xlab("Search + Removal Alternative")+ ylab("Average estimated final state") +
  labs(title = "Final average true state across sites and simulations (Generating)")
