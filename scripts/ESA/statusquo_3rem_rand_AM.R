...
path <- here::here("results", "ESA", "statusquo_rand")
res <- c('results/ESA/statusquo_rand') #subset of path for plot save
...
site.selected <- array(NA, dim = c(n.rem, n.years, n.sims))

for(s in 1:n.sims){
  for(year in 1:n.years){
    site.selected[,year,s] <- sample(seq(1,5), n.rem, replace = F)
  }
}

year <- 1
for(s in 1:n.sims){
  for(i in site.selected[,year,s]){
