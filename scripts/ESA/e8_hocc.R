...
#### Path Name ####
path <- here::here("results", "ESA", "e8_hocc")
...
psi1 <- -2 #effect of previous removal
psi2 <- 1  #effect of site characteristics 
...
for(year in 1:n.years){
  ...
  if(year > 1){ #we simulated year 1 data above
    ...
    for(s in 1:n.sims){
      for(i in 1:n.sites){
        psi[i,year,s]  <- invlogit(psi0 + psi1*previous.rem[i,year,s]+ psi2*site.char[i])
  ...
  for(s in 1:n.sims){
    effort.dat[rem.vec[,year,s] >= 0,1:2,s] <- search.effort[1,year,s] 
  }
  
  #Parameters monitored
  parameters.to.save <- c("psi.est", "psi0", "psi1", "psi2", "a0","a1", "State")
  ...
  #1. find effort where detection probability = 0.8
  
  det.p <- log(0.8/(1-0.8))
  
  if(year < n.years){
    for(s in 1:n.sims){
      a0.sim[s] <- get(a0s[s])$mean #mean value cannot be 0 for division
      a1.sim[s] <-  get(a1s[s])$mean #mean value cannot be 0 for division
      
      search.effort[,year+1,s] <- exp((det.p - a0.sim[s])/a1.sim[s])
    }
    ...
    for(s in 1:n.sims){
      ...
      for(i in 1:n.sites){
        psi[i,n.years+1,s]  <- invlogit(psi0 + psi1*previous.rem[i,n.years,s]+ psi2*site.char[i])
...
file_name = paste(path, 'States_e8_hocc.csv',sep = '/')
...
file_name = paste(path, 'Mean.States_e8_hocc.csv',sep = '/')
...
file_name = paste(path, 'y.obs_e8_hocc.csv',sep = '/')
write.csv(y.obs.df,file_name)


rem.site.df <- y.obs.df %>% filter(observed.state == 1)
file_name = paste(path, 'rem.site_e8_hocc.csv',sep = '/')
...
file_name = paste(path, 'States.est_e8_hocc.csv',sep = '/')
...
file_name = paste(path, 'Mean.States.est_e8_hocc.csv',sep = '/')
write.csv(Mean.States.est.df ,file_name)

##### Estimated effort ####
search.effort.df <- adply(search.effort[1,,], c(1,2))

colnames(search.effort.df) <- c("year", "sim", "effort")              

file_name = paste(path, 'effort_e8_hocc.csv',sep = '/')
write.csv(search.effort.df,file_name)
...
file_name = paste(path, 'psis.est_e8_hocc.csv',sep = '/')
...
file_name = paste(path, 'psi0s.est.est_e8_hocc.csv',sep = '/')
...
file_name = paste(path, 'psi1s.est.est_e8_hocc.csv',sep = '/')
...
file_name = paste(path, 'psi2s.est.est_e8_hocc.csv',sep = '/')
...
file_name = paste(path, 'a0s.est.est_e8_hocc.csv',sep = '/')
...
file_name = paste(path, 'a1s.est.est_e8_hocc.csv',sep = '/')
write.csv(a1s.est.df,file_name)

###### rhat vals ######
file_name = paste(path, 'rhat.vals_e8_hocc.csv',sep = '/')
...
file_name = paste(path, 'e8_hocc_time.txt',sep = '/')
