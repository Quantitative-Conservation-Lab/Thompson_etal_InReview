...
State.init <- State.init# rep(1,n.sites)
...
pM.l <- array(NA, c(n.sites, n.weeks,n.years, n.sims,n.alts))
pM.h <- array(NA, c(n.sites, n.weeks,n.years, n.sims,n.alts))
...
logsearch.L.L.5 <- (logit(0.5) - B0.pl)/(B1.pl)
logsearch.L.H.5 <- (logit(0.5) - B0.ph)/(B1.ph)
logsearch.L.L.75 <- (logit(0.75) - B0.pl)/(B1.pl)
logsearch.L.H.75 <- (logit(0.75) - B0.ph)/(B1.ph)
    ...
    #  pM.h[1:n.sites,w,y,s]  <- invlogit(B0.ph[s] + B1.ph[s]*logeffortsearch[s]) #high state detection 
      for(a in 1:n.alts){
        for(i in 1:n.sites){
          ...
          P.datM[2,i,w,y,s,a,] <- c(0,1,0)#c(1-pM.l[i,w,y,s], pM.l[i,w,y,s], 0)
          P.datM[3,i,w,y,s,a,] <- c(0,0,1)#c(1-pM.h[i,w,y,s], pM.h[i,w,y,s]*(1-delta[s]), pM.h[i,w,y,s]*delta[s])
        } 
      }
...
StateD <- array(NA, c(n.sites, n.weeks, n.years, n.sims, n.alts))
...
for(year in 1:n.years){
  for(a in 1:n.alts){
    ...
    if(year == 1){
      ...
      StateD[,1,year, 1:n.sims,a] <- State[,1,year, 1:n.sims,a]
      StateD[,1,year, 1:n.sims,a][State[,1,year, 1:n.sims,a] == 2] <- 4
      sites.rem.M[,1,year,s,a] <- order(StateD[,1,year,s,a], decreasing = T)
    }else{
      for(s in 1:n.sims){
        for(i in 1:n.sites){ #State process: state given previous state and transition probability
          ...
          StateD[i,1,year,s,a] <- State[i,1,year,s,a]
          StateD[i,1,year,s,a][State[i,1,year,s,a] == 2] <- 4
           }
...
#################################################################################################
#### Path to save data ####
path <- 'E:\\Chapter3\\results-space2\\hstate_gen\\40_datM'
...
test <- mean(S.fin$state[1:4000])
test2 <- mean(S.fin$state[4001:8000])
