...
delta <- 0.8 #probability of observing the high state given detected and true state is high
...
for(year in 1:n.years){
  ...
  if(year == 1){
    ...
    for(s in 1:n.sims){
      ...
      # --- delta --- perfectly detecting high state --------#
  ...
  parameters.to.save <- c("eps.l0", "eps.l1", "eps.h0", "eps.h1", "gamma.0", "gamma.1",
                          ...
                          "p.l0", "p.l1", "p.h0", "p.h1", "alpha.l", "alpha.h", "psi", "delta", "State.fin")
  ...
  for(s in 1:n.sims){
    for(i in 1:n.sites){
      if(rem.vec.dat[i,1,s] == 1){
        State.start[i,1,s] <- max(yM[i,,1,year,s], na.rm = T)
      }else{
        State.start[i,1,s] <-  2
      }
      for(week in 2:n.weeks){
        ...
        if(rem.vec.dat[i,week-1,s] == 1){
          State.start[i,week,s] <-  max(max(ytest[i,,week,s], na.rm = T), 1, na.rm = T)
        }
