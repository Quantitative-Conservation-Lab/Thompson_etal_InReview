...
for(p in 1:n.params){
  pM.l[p] <- invlogit(p.l0s[p] + p.l1s[p]*logsearch.effort + alpha.ls[p]) #low state detection probability (base detection + effect of effort)
  pM.h[p] <- invlogit(p.h0s[p] + p.h1s[p]*logsearch.effort + alpha.hs[p]) #high state detection probability (base detection + effect of effort)
