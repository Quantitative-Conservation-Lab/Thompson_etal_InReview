...
for (i in 1:n.sites){  
  ...
  for(y in 1:2){
    for (t in 1:4){
      ...
      logit(eps.l[i,t,y]) <- eps0.l + eps1.l*rem.vec[i,t,y]*removal.hours #erradication low state, rem.vec = indicator of removal
      logit(eps.h[i,t,y]) <- eps0.h + eps1.h*rem.vec[i,t,y]*removal.hours #erradication high state
    
      logit(phi.hh[i,t,y]) <- phi0.hh - phi1.hh*rem.vec[i,t,y]*removal.hours
      ...
      TPM[2,i,t,y,3] <- (1- eps.l[i,t,y]) #low to high
    
      TPM[3,i,t,y,1] <- eps.h[i,t,y] #high to empty
      TPM[3,i,t,y,2] <- (1- eps.h[i,t,y])*(1-phi.hh[i,t,y]) #high to low
      TPM[3,i,t,y,3] <- (1- eps.h[i,t,y])*(phi.hh[i,t,y]) #high to high
    
    } #t
  } #y
  
     logit(gamma[i]) <-gamma0 + gamma1*site.char[i] + gamma2*D[i,5,1] #invasion probability 
      
      #Between week calculation
      TPM[1,i,5,1,1] <-  1- gamma[i] #empty to empty
      TPM[1,i,5,1,2] <-  gamma[i] #empty to low
      TPM[1,i,5,1,3] <-  0 #empty to high

      TPM[2,i,5,1,1] <- epsB.l #low to empty
      TPM[2,i,5,1,2] <- (1-epsB.l)*(1-phiB.lh) #low to low
      TPM[2,i,5,1,3] <-  (1-epsB.l)*(phiB.lh) #low to high
    
      TPM[3,i,5,1,1] <-  epsB.h #high to empty
      TPM[3,i,5,1,2] <-  (1-epsB.h)*(1-phiB.hh) #high to low
      TPM[3,i,5,1,3] <-  (1-epsB.h)*(phiB.hh) #high to high
...
#-----------------------------------------------------------------------------#
  ...
  for (i in 1:n.sites){
    ...
    D[i,5,1] <- sum(State[neighbors[i,], 5,1])/n.neighbors[i] #state of neighbors 
