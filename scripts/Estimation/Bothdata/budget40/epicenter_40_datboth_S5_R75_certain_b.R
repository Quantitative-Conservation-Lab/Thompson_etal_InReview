...
res <- 'E:/Chapter3/results_datboth/budget40/epicenter_S5_R75_40_certain_b/densplots'
#------------------------------------------------------------------------------#
#### Management Strategy ####
...
for(s in 1:n.sims){
  pD.l[s] <- 1 #invlogit(B0.pl[s] + B1.pl[s]*logeffort.D + 0 ) #alpha.l[s])
  pD.h[s] <- 1 #invlogit(B0.ph[s] + B1.ph[s]*logeffort.D + 0 ) #alpha.h[s])
...
for(year in 2:n.years){
  ...
  if(year == 2){
    ...
    file.name <- "Flower_datB_est1_explorecert.txt"
  ...
  if(year %in% c(3:last.explore)){
    ...
    file.name <- "Flower_datB_est2_explorecert.txt"
  }
  ...
  if(year > last.explore){
    ...
    file.name <- "Flower_datB_est2_cert.txt"
...
for(s in 1:n.sims){
  ...
  truth.paramslist[[s]]$mean <- c(0,0,1,1,2,
                                  2,0,1,0,-2,0, 
                                  (3/15), (3/15), 14/30, 12/24, 3.5/35,
                                  0,0,0,0,0.5)
  
  truth.paramslist[[s]]$low <- c(qnorm(0.025, 0, 0.5),qnorm(0.025, 0, 0.5),
                                 qnorm(0.025, 1, 0.05), qnorm(0.025, 1, 0.05), qnorm(0.025, 2, 0.05),
  ...
  truth.paramslist[[s]]$high <- c(qnorm(0.975, 0, 0.5),qnorm(0.975, 0, 0.5),
                                  qnorm(0.975, 1, 0.05), qnorm(0.975, 1, 0.05), qnorm(0.975, 2, 0.05),
