...
sink("Flower_datB_est1_explore.txt")
cat("
model{
# -------------------------------------------------
# States (S):
...
#### Model 2 ####
sink("Flower_datB_est2_explore.txt")
cat("
model{
# -------------------------------------------------
# States (S):
...
#difference between detection/nondetection data and multistate data
B0.p.l.D ~ dnorm(B0.p.l.D.mean, B0.p.l.D.sd)
B0.p.h.D ~ dnorm(B0.p.h.D.mean, B0.p.h.D.sd)


#Detection/nondetection data
logit(pD.l) <- B0.p.l.D #detection low state 
logit(pD.h) <- B0.p.h.D #detection high state
