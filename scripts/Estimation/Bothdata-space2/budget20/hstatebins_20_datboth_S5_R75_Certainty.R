...
##### Path to save data ####
path <- 'E:\\Chapter3\\results-datboth-space2\\budget20\\hstatebins_S5_R75_cert'
res <- 'E:/Chapter3/densplots/results-datboth-space2/budget20/hstatebins_S5_R75_cert'
...
for(s in 1:n.sims){
  ...
  pM.l[s] <- 1#invlogit(B0.pl[s] + B1.pl[s]*logsearch.effort[s]) #low state detection 
  pM.h[s]  <- 1#invlogit(B0.ph[s] + B1.ph[s]*logsearch.effort[s]) #high state detection 
...
for(year in 1:n.years){
  ...
  #sites where removal occurred
    ...
    file.name <- "Flower_datB_estCERT.txt"
