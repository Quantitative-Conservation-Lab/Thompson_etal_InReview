...
for(year in 1:n.years){
  ...
  TPM[2,1:n.sites,1,year,p,,1] <- eps.l[,1,year,p,] #low to empty 
  TPM[2,1:n.sites,1,year,p,,2] <- (1- eps.l[,1,year,p,]) #low to low 
  TPM[2,1:n.sites,1,year,p,,3] <- 0 #low to high 
    
  TPM[3,1:n.sites,1,year,p,,1] <- eps.h[,1,year,p,] #high to empty 
  TPM[3,1:n.sites,1,year,p,,2] <- (1- eps.h[,1,year,p,])*(1-phi.hh[,1,year,p,]) #high to low 
  TPM[3,1:n.sites,1,year,p,,3] <- (1- eps.h[,1,year,p,])*(phi.hh[,1,year,p,]) #high to high 
  ...
  #Between weeks: we need to project 48 weeks forward
    ...
    gamma[,week,year,p,s] <-invlogit(gamma0[p] + gamma1[p]*site.char + gamma2[p]*D[,week,year,p,s]) #invasion
