...
#------------------------------------------------------------------------------#
#### Path to save data ####
path <- here::here("results", "explore_settle_linear", "SrL_RrL_30b")
res <- c('results/explore_settle_linear/SrL_RrL_30b') #subset of path for plot save
#------------------------------------------------------------------------------#
#### Management Strategy ####
...
for(year in 2:n.years){
  ...
  if(year == last.explore){
    ...
    logsearch.effort.L <- (logit(0.2) - unlist(B0.p.l.est))/unlist(B1.p.l.est)
    logsearch.effort.H <- (logit(0.2) - unlist(B0.p.h.est))/unlist(B1.p.h.est)
    
    removal.L <- (logit(0.2) - (unlist(B0.eps.l.est)))/((unlist(B1.eps.l.est)))
    removal.L <- removal.L[removal.L > 0]
    removal.H <-(logit(0.2) - (unlist(B0.eps.h.est)))/((unlist(B1.eps.h.est)))
